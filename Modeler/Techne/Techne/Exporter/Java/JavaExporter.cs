...
{
    [PluginExport("Turbo Model Thingy", "0.5", "ZeuX", "Allows models to be exported as java", "1CD6C3E3-E9EA-419C-BC1B-3E12D48E554D")]
    [Export(typeof(IExportPlugin))]
    public class JavaExporter : IExportPlugin
    {
        private readonly Dictionary<string, MemberDescriptor> exportDefinitions = new Dictionary<string, MemberDescriptor>();
        ...
        private void CreateContents(IEnumerable<ITechneVisual> visuals, IndendingStringBuilder constructorContents, IndendingStringBuilder renderContents, IndendingStringBuilder fieldDefinitions, IndendingStringBuilder animationCode)
        {
            foreach (var visual in visuals)
            {
                ...
                {
                    CreateContents((visual as ModelVisual3D).Children.Cast<ITechneVisual>(), constructorContents, renderContents, fieldDefinitions, animationCode);
                }
                else
                {
        ...
        private void CreateAnimationCode(ITechneVisual visual, IndendingStringBuilder animationCode)
        {
            if (visual.AnimationAngles.X != 0 && visual.AnimationDuration.X > 0)
            {
                ...
                animationCode.Append(((int)visual.AnimationType.X + 1).ToString());
                animationCode.Append(" + ");
                animationCode.Append(ToRad(visual.RotationX).ToString().Replace(',', '.'));
                animationCode.Append(";");
            }
            ...
            if (visual.AnimationAngles.Y != 0 && visual.AnimationDuration.Y > 0)
            {
                ...
                animationCode.Append(" * f");
                animationCode.Append(((int)visual.AnimationType.Y + 1).ToString());
                animationCode.Append(" + ");
                animationCode.Append(ToRad(visual.RotationY).ToString().Replace(',', '.'));
                animationCode.Append(";");
            }
            ...
            if (visual.AnimationAngles.Z != 0 && visual.AnimationDuration.Z > 0)
            {
                ...
                animationCode.Append(" * f");
                animationCode.Append(((int)visual.AnimationType.Z + 1).ToString());
                animationCode.Append(" + ");
                animationCode.Append(ToRad(visual.RotationZ).ToString().Replace(',', '.'));
                animationCode.Append(";");
            }
        }
        ...
        private void CreateVisualConstructorContents(IndendingStringBuilder constructorContents, ITechneVisual visual, IndendingStringBuilder fieldDefinitions)
        {
            ...
            if (!visual.IsDecorative)
            {
                var constructor = exportDefinitions[visual.GetType().ToString()].ConstructorEntry;
                fieldDefinitions.Append("public ");
                fieldDefinitions.Append(constructor[0].Attribute.Name);
        ...
        private string SetField(ITechneVisual visual, Entry item)
        {
            ...
            return String.Format("{0}.{1} = {2};",
                                 visual.Name,
                                 item.Attribute.Name,
                                 TypeToJavaHelper.Convert(item.Property.GetValue(visual, null), item.Property.PropertyType, item.Attribute));
        }
