...
namespace HelixToolkit
{
    ...
    public class MeshBuilder
    {
        ...
        /// <param name="width">The width of the face.</param>
        /// <param name="height">The height of the face.</param>
        public void AddCubeFace(Point3D center, Vector3D normal, Vector3D up, double dist, double width, double height, Point[] texture)
        {
            var right = Vector3D.CrossProduct(normal, up);
            var n = normal * dist / 2;
            up *= height / 2;
            ...
            if (textureCoordinates != null)
            {
                if (texture == null || texture.Length != 4)
                {
                    texture = new Point[4];
                    texture[0] = new Point(1, 1);
                    texture[1] = new Point(0, 1);
                    texture[2] = new Point(0, 0);
                    texture[3] = new Point(1, 0);
                }

                textureCoordinates.Add(texture[0]);
                textureCoordinates.Add(texture[1]);
                textureCoordinates.Add(texture[2]);
                textureCoordinates.Add(texture[3]);
            }
            triangleIndices.Add(i0 + 2);
            triangleIndices.Add(i0 + 1);
            triangleIndices.Add(i0 + 0);
